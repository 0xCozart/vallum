...
3. The demo dApp creates a `@vallum/sdk` client with the local app key.
...
The browser wrapper is a small loopback-only HTTP server that serves a page plus a same-origin `/api/run-demo` endpoint. The browser never receives the app key; the local server keeps `VALLUM_DEMO_APP_KEY` server-side and returns sanitized success/error JSON. Cross-origin POST attempts and non-empty request bodies are rejected before any gateway call.
