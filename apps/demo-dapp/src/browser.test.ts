...
import { VallumPolicyError } from "@vallum/sdk";
...
test("renderDemoBrowserPage renders a same-origin demo runner without credentials", () => {
  ...
  assert.doesNotMatch(html, /local-dev-demo-key|Bearer|VALLUM_DEMO_APP_KEY/i);
});
...
test("demo browser server serves the page and health endpoint", async () => {
  await withServer({ runFlow: async () => ({ reservationId: "unused", agentRailTransactionId: "unused", digest: "unused" }) }, async (baseUrl) => {
    ...
    assert.match(await page.text(), /Vallum Local Demo/);
  ...
  await withServer({
    runFlow: async () => {
      ...
      return {
        ...
        agentRailTransactionId: "vallum_tx_1",
...
test("demo browser server sanitizes SDK errors without exposing raw upstream bodies", async () => {
  await withServer({
    runFlow: async () => {
      throw new VallumPolicyError("Package not allowed for local-dev-demo-key", "PACKAGE_NOT_ALLOWED", 400, {
        bearer: "secret-token",
      });
    },
  }, async (baseUrl) => {
...
test("demo browser server refuses non-loopback hosts and invalid ports", async () => {
  await assert.rejects(
    () => startDemoBrowserServerFromEnv({ VALLUM_DEMO_DAPP_HOST: "0.0.0.0", VALLUM_DEMO_DAPP_PORT: "0" }),
    /loopback/i,
  );

  await assert.rejects(
    () => startDemoBrowserServerFromEnv({ VALLUM_DEMO_DAPP_HOST: "127.0.0.1", VALLUM_DEMO_DAPP_PORT: "not-a-port" }),
    /valid port/i,
  );
});
