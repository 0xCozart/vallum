...
const outDir = checkOnly ? resolve(repoRoot, "tmp/vallum/docs-site-check") : resolve(appRoot, "dist");
...
function htmlToken(placeholders, html) {
  const token = `__VALLUM_HTML_${placeholders.length}__`;
...
async function build() {
  ...
  if (checkOnly) {
    ...
    if (!examples.includes("createVallumClient") || !examples.includes("fetch(&quot;/api/vallum/reserve&quot;")) {
      throw new Error("Docs check failed: code examples page did not render expected SDK and browser examples.");
    }
    if (!overview.includes("data-theme-toggle") || !overview.includes("vallum-docs-theme")) {
