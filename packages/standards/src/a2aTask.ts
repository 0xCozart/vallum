...
function createTask(
  ...
  if (options.manifest === undefined) {
    throw new A2ATaskError("A2A_MANIFEST_REQUIRED", "New A2A tasks require an Agentic GasKit manifest.");
  }
  if (!options.policy) {
    throw new A2ATaskError("A2A_POLICY_REQUIRED", "New A2A tasks require an Agentic GasKit policy.");
  }
...
async function continueTask(
  ...
  return processAndStoreTask(options, nextTask, message, now, undefined, task.agenticGasKit?.policyDecision);
}
  ...
  const outcome = options.processMessage
    ...
    : {
        ...
        artifacts: [{
          ...
          parts: [{ text: "A2A task accepted by local Agentic GasKit runtime." }],
        }],
      };
...
function rejectionMessage(policyDecision: Exclude<AgentPolicyDecision, { allowed: true }>, now: Date): A2AMessage {
  return {
    ...
    role: "ROLE_AGENT",
    parts: [{ text: `Agentic GasKit policy denied the A2A task: ${policyDecision.reasonCode}` }],
    metadata: {
