---
name: vallum
description: Use when working in the vallum repo or integrating Vallum in another app, and when extending the Vallum fork, including repo navigation, SDK/gateway/demo/docs changes, local verification, agent migration docs, wallet safety, testnet readiness, sponsor-gas safety, or IOTA sponsored transaction flows.
---
...
Use this skill to work on Vallum without losing the repo's safety boundaries.
Vallum is built on the original IOTA GasKit sponsorship foundation around
the official IOTA Gas Station: app backends call the SDK or policy gateway,
Vallum applies app auth and sponsorship policy, and only allowed requests
reach IOTA Gas Station.

Vallum extends that foundation with agent accounts, signer references,
...
1. Read `AGENTS.md`.
2. Read `docs/vallum/migration-plan.md` if it exists.
...
- Agentic migration: `docs/vallum/migration-plan.md`,
  `docs/vallum/roadmap.md`, `docs/vallum/execution-slices.md`,
...
- Keep browser flows thin. Browser code calls same-origin backend routes; backend routes own Vallum app credentials.
