...
Set `VALLUM_TESTNET_DIGEST_REPORT=tmp/vallum/testnet-digest-proof.json`
...
When `VALLUM_POLICY_PATH` is relative, this CLI resolves it relative to the selected env file directory before falling back to the repo working directory.
...
- `VALLUM_GATEWAY_HOST` remains loopback-only for the first testnet demo boundary.
...
- If reserve compatibility fails after Gas Station root and IOTA RPC checks
  ...
  tmp/vallum/sponsor-funding-request.json` can write the public sponsor address
  to an ignored local artifact for operator funding while keeping stdout
  redacted. If `--faucet-report <ignored-json-path>` or
  `VALLUM_SPONSOR_FAUCET_REPORT` is set, the artifact can include only bounded
  ...
  tmp/vallum/sponsor-faucet-request.json` can request testnet gas from
  `IOTA_FAUCET_URL` or `--faucet-url`; it requires explicit `--execute`.
  Then `npm run sponsor:check-funding -- --report
  tmp/vallum/sponsor-funding-report.json` reports whether the configured
  ...
  full sponsor address. Point `VALLUM_SPONSOR_FUNDING_REPORT` at that ignored
  report when running the non-networked proof gates.
- `VALLUM_TESTNET_UPSTREAM_REPORT` points at a current sanitized report created
...
or compatible; `VALLUM_TESTNET_UPSTREAM_REPORT` must still point at a current
...
The report template is not accepted by `VALLUM_TESTNET_UPSTREAM_REPORT`. It
...
tmp/vallum/sponsor-funding-request.json` can be used when the operator needs
...
tmp/vallum/sponsor-faucet-request.json` can be used only when the operator has
...
If `VALLUM_SPONSOR_FAUCET_REPORT` points at the ignored faucet report, or if
the default ignored `tmp/vallum/sponsor-faucet-request.json` report exists and
...
`VALLUM_SPONSOR_FUNDING_REPORT` evidence artifact before the upstream
diagnostic step, while keeping `VALLUM_SPONSOR_FAUCET_REPORT` optional.
...
tmp/vallum/sponsor-funding-report.json` can be used as a read-only funding
...
tmp/vallum/iota-identity-live-report.json` is the report-backed IOTA Identity
