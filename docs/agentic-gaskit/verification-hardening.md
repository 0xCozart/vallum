...
- Humans/operators fund agent wallets directly or through GasKit sponsorship;
...
### Blocker: Existing GasKit Can Be Accidentally Rebuilt

Failure mode:
Agentic GasKit duplicates the existing GasKit gateway, SDK, deployment,
sponsor-wallet, and testnet-readiness surfaces instead of extending them.

Impact:
The project forks its own safety boundary, loses proven GasKit behavior, and
creates inconsistent policy, credentials, quota, and sponsor-wallet semantics.

Fix:
Start with Slice 0.3. Treat existing IOTA GasKit as the canonical sponsorship
foundation inside this fork unless a migration or adapter reason is explicitly
documented.

Validation:
Docs and code identify reused existing GasKit surfaces, and duplicated gateway
...
- existing GasKit integration map exists or duplicated sponsorship
...
- a product rename away from Agentic GasKit
