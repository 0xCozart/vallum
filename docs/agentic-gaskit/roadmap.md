# Agentic GasKit Development Roadmap
...
| Agentic GasKit | Preferred product name | High | Use as implementation/package naming unless changed. |
...
Goal:
Create an end-to-end executable development roadmap for Agentic GasKit from the
raw IOTA AgentKit thesis and the existing GasKit implementation in this fork.
...
Current problem:
The staging repo previously treated GasKit as something to build from scratch,
...
- explicit integration boundary with existing IOTA GasKit
...
- Reuse existing IOTA GasKit for Gas Station sponsorship, app credentials,
...
Agentic GasKit should not rebuild those surfaces by default. It should add the
...
workflow demos that call into or adapt the existing GasKit foundation.
...
Agent app / LLM / automation runtime
  ...
  -> IOTA integration layer
       - existing IOTA GasKit / Gas Station sponsorship
...
| Existing IOTA GasKit integration | Phase 0 then all phases | Reuse canonical GasKit gateway, SDK, deployment, testnet readiness, and sponsor-wallet safety | current fork |
| Account/wallet manager | Phase 1 then expanded | Agent-created wallets, signer references, safe storage adapters, rotation/revocation | Existing GasKit safety model |
...
Start with Phase 0 and an integration pass against the existing GasKit code in
...
7. expose a local policy gateway API or adapter to existing IOTA GasKit
