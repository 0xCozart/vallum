...
| A2A public readiness | `npm run proof:a2a-public-readiness` classifies local A2A proof, local authenticated extended-card access, local loopback streaming, local push notification configuration, local injected push delivery, local opt-in push HTTP transport, local retry/attempt observability, public hosting inputs, production JWKS/auth decisions, redacted structured public push delivery evidence, and structured external conformance blockers without contacting public endpoints. | [A2A Public Readiness](agentic-gaskit/a2a-public-readiness.md) |
