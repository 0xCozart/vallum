...
review of live facilitator, processor, or settlement claims.
...
| Payment provider readiness | `npm run proof:payment-provider-readiness` validates local x402/AP2 source and tests plus an optional ignored structured report path, without contacting payment providers or printing report paths or secret-like fields. | [Product Status Proof](agentic-gaskit/product-status.md) |
