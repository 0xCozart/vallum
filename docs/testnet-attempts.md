...
- Package publish dry-run passed for `@vallum/shared-types`, `@vallum/policy-gateway`, and `@vallum/sdk`.
...
- `vallum-redis`
- `vallum-gas-station`, bound to `127.0.0.1:9527`
...
Gateway reserve through the local policy gateway now succeeds against the real Gas Station upstream and returns sponsor address, reservation id, gas coins, and a Vallum transaction id.
...
agentRailTransactionId=vallum_5...3b726
...
the Vallum ignored `.env` was updated locally, the Gas Station config
...
reservedGas=true
reservationId=<redacted-id>
agentRailTransactionId=vallum_b...f2234e
sponsorAddress=0xd046a4fb...29b9b868
executed=true
...
`-- --report tmp/vallum/sponsored-execute-report.json` to write an ignored
...
```bash
npm run execute:testnet-demo -- --report tmp/vallum/sponsored-execute-report-20260614.json
```

Sanitized result:

```text
reportKind=vallum.sponsored-testnet-execute-report
result=passed
...
reservedGas=true
reservationId=<redacted-id>
agentRailTransactionId=vallum_e...787967
sponsorAddress=0xd046a4fb...29b9b868
executed=true
...
current Vallum checkout was confirmed to have a funded testnet sponsor
...
reservedGas=true
reservationId=<redacted-id>
agentRailTransactionId=vallum_1...a4724c
sponsorAddress=0xd046a4fb...29b9b868
executed=true
...
```bash
npm run execute:testnet-demo -- --report tmp/vallum/sponsored-execute-report-20260614-refresh.json
```
...
reservedGas=true
reservationId=<redacted-id>
agentRailTransactionId=vallum_5...bbad51
sponsorAddress=0xd046a4fb...29b9b868
executed=true
...
report=tmp/vallum/testnet-upstream-diagnostic.json
```
...
tmp/vallum/testnet-upstream-diagnostic.json` without `--skip-reserve`.
...
```text
Vallum direct Docker Gas Station status
ready=true
...
## 2026-06-14 upstream reachability classification refresh

The upstream diagnostic was refreshed after adding bounded Gas Station
...
The ignored JSON report now records `gasStationReachability.code` and keeps
the optional wrapper health 404 separate from upstream reachability. The
report remains blocked because reserve_gas compatibility still fails while the
configured sponsor funding report is blocked with
