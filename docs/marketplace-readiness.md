...
Vallum is ready for marketplace requirements/design work that stays
...
| Contract metadata, pay-per-call, data-license, service-bounty, reputation receipts, and subscriptions | Local template metadata allow-listing, paid tool, data-license, service-bounty, reputation-receipt, and subscription flows pass with denial and failure cases. Device access has a safety gate, not an implementation. | Marketplace can model supported templates and local paid/data-license/service-bounty/reputation/subscription evidence calls, but broader contract workflows, recurring billing, dispute operation, public scoring, production provider access, and device access are not complete. |
| x402, AP2, and A2A bridges | Local mapping, receipt linking, well-known response, signed Agent Card verification, task/message operation, local HTTP-shaped A2A boundary, loopback A2A server tests, and `npm run proof:payment-provider-readiness` pass for supported local evidence; unsupported versions fail closed; sensitive metadata redaction is tested. | Marketplace can show standards compatibility as local bridge evidence only, not live facilitator, AP2 network, live public A2A server, production Agent Card key management, external conformance, or public A2A discovery proof. |
...
| Live payment settlement | Unproven live by default. `npm run proof:payment-provider-readiness` can validate an ignored redacted structured report before manual review, but it does not run provider calls itself. | Prove x402/AP2/payment processor paths with safe credentials, partial-failure receipt handling, and accepted structured report evidence. |
...
| Device access safety gate | Physical device operation is blocked; any future proof must start with virtual or simulated resources only. | Approve physical safety, provider accountability, revocation, emergency-stop, credential, privacy, dispute, compliance, and incident response design before any device marketplace claim. |
...
tmp/vallum/marketplace-readiness.json` writes the same readiness state as a
...
tmp/vallum/marketplace-production-proof-bundle.json` writes the report
...
- Future marketplace work consumes existing Vallum primitives instead
