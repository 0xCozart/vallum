...
| x402, AP2, and A2A bridges | Local mapping, well-known response, task/message operation, and local HTTP-shaped A2A boundary tests pass for supported versions; unsupported versions fail closed; sensitive metadata redaction is tested. | Marketplace can show standards compatibility as local bridge evidence only, not live facilitator, AP2 network, live public A2A server, signed Agent Card, external conformance, or public A2A discovery proof. |
