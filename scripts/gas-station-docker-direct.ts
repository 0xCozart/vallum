...
const DEFAULT_GAS_STATION_CONTAINER = "vallum-gas-station";
const DEFAULT_GAS_STATION_IMAGE = "iotaledger/gas-station:latest";
const DEFAULT_NETWORK = "vallum-local";
const DEFAULT_REDIS_CONTAINER = "vallum-redis";
...
export function formatGasStationDockerDirectPlan(plan: GasStationDockerDirectPlan): string {
  const lines = [
    "Vallum direct Docker Gas Station plan",
...
export function formatGasStationDockerDirectStatus(report: GasStationDockerDirectStatusReport): string {
  ...
  lines.push(report.ready
    ? "next=npm run diagnose:gas-station -- --report tmp/vallum/testnet-upstream-diagnostic.json"
...
async function main(): Promise<number> {
  ...
  if (options.execute) {
    ...
    console.log("next=npm run diagnose:gas-station -- --report tmp/vallum/testnet-upstream-diagnostic.json");
  } else {
