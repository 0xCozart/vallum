...
export interface LiveProofStatusArtifact {
  ...
  readonly kind: "vallum.live-proof-status-report";
...
const SPONSOR_FUNDING_REPORT_ENV = "VALLUM_SPONSOR_FUNDING_REPORT";
const SPONSOR_FAUCET_REPORT_ENV = "VALLUM_SPONSOR_FAUCET_REPORT";
const DEFAULT_SPONSOR_FAUCET_REPORT = "tmp/vallum/sponsor-faucet-request.json";
const TESTNET_UPSTREAM_REPORT_ENV = "VALLUM_TESTNET_UPSTREAM_REPORT";
const TESTNET_DIGEST_REPORT_ENV = "VALLUM_TESTNET_DIGEST_REPORT";
const IOTA_NAMES_LIVE_REPORT_ENV = "IOTA_NAMES_LIVE_REPORT";
const IOTA_IDENTITY_LIVE_REPORT_ENV = "IOTA_IDENTITY_LIVE_REPORT";
const TESTNET_UPSTREAM_TEMPLATE_COMMAND = "npm run operator:write-report-template -- --kind testnet-upstream --out tmp/vallum/testnet-upstream-report-template.json";
const TESTNET_DIGEST_TEMPLATE_COMMAND = "npm run operator:write-report-template -- --kind testnet-digest --out tmp/vallum/testnet-digest-report-template.json";
const IDENTITY_PROOF_BUNDLE_COMMAND = "npm run live:write-identity-proof-bundle -- --out tmp/vallum/identity-proof-bundle.json";
const IOTA_NAMES_TEMPLATE_COMMAND = "npm run operator:write-report-template -- --kind iota-names-live --out tmp/vallum/iota-names-live-report-template.json";
const IOTA_IDENTITY_TEMPLATE_COMMAND = "npm run operator:write-report-template -- --kind iota-identity-live --out tmp/vallum/iota-identity-live-report-template.json";
const VC_VALIDATION_TEMPLATE_COMMAND = "npm run operator:write-report-template -- --kind vc-validation-live --out tmp/vallum/vc-validation-live-report-template.json";
...
export function buildLiveProofStatusArtifact(
  ...
  return {
    schemaVersion: 1,
    kind: "vallum.live-proof-status-report",
...
async function checkGasStationRuntimeStatus(
  ...
  return {
    ...
    next: report.code === "GAS_STATION_MANAGED_UPSTREAM_CONFIG_MISSING"
      ? "Set GAS_STATION_URL outside committed files or switch VALLUM_GAS_STATION_RUNTIME_MODE back to local-docker, then rerun npm run gas-station:runtime-preflight."
...
export function formatLiveProofStatusReport(report: LiveProofStatusReport): string {
  const lines = [`Vallum live proof status ${report.ok ? "ready" : "blocked"}`];
...
async function checkIotaNamesStatus(
  ...
  if (missing.length > 0) {
    return {
      ...
      next: `${IDENTITY_PROOF_BUNDLE_COMMAND}; ${IOTA_NAMES_TEMPLATE_COMMAND}; set the missing variables outside committed files, then run npm run smoke:iota-names-live -- --report <ignored-json-path>.`,
    };
  }
  ...
  if (!reportPath) {
    return {
      ...
      next: `${IDENTITY_PROOF_BUNDLE_COMMAND}; ${IOTA_NAMES_TEMPLATE_COMMAND}; run npm run smoke:iota-names-live -- --report <ignored-json-path> with operator approval, then rerun this gate.`,
    };
  }
...
async function checkIotaIdentityStatus(
  ...
  if (missing.length > 0) {
    return {
      ...
      next: `${IDENTITY_PROOF_BUNDLE_COMMAND}; ${IOTA_IDENTITY_TEMPLATE_COMMAND}; set the missing variables outside committed files, then run npm run smoke:iota-identity-live -- --report <ignored-json-path>.`,
    };
  }
  ...
  if (!reportPath) {
    return {
      ...
      next: `${IDENTITY_PROOF_BUNDLE_COMMAND}; ${IOTA_IDENTITY_TEMPLATE_COMMAND}; run npm run smoke:iota-identity-live -- --report <ignored-json-path> with operator approval, then rerun this gate.`,
    };
  }
...
async function checkVcTrustPolicyStatus(
  ...
  if (missing.length > 0) {
    return {
      ...
      next: `${IDENTITY_PROOF_BUNDLE_COMMAND}; ${VC_VALIDATION_TEMPLATE_COMMAND}; set the missing IOTA Identity trust-policy variables outside committed files before accepting live VC proof for policy-gated actions.`,
    };
  }
  ...
  if (!reportPath) {
    return {
      ...
      evidence: `missing=${IOTA_IDENTITY_LIVE_REPORT_ENV}`,
      next: `${IDENTITY_PROOF_BUNDLE_COMMAND}; ${VC_VALIDATION_TEMPLATE_COMMAND}; run npm run smoke:iota-identity-live -- --report <ignored-json-path> with operator approval, then rerun this gate.`,
    };
  }
