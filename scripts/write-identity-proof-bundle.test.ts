...
test("identity proof bundle writes templates, plan, and blocked summary without secret values", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "vallum-identity-bundle-"));
  try {
    ...
    const bundleRaw = await readFile(join(cwd, "tmp/vallum/identity-proof-bundle.json"), "utf8");
    const planRaw = await readFile(join(cwd, "tmp/vallum/live-proof-plan.json"), "utf8");
    const liveStatusRaw = await readFile(join(cwd, "tmp/vallum/live-proof-status.json"), "utf8");

    assert.equal(bundle.kind, "vallum.identity-proof-bundle");
    ...
    assert.equal(bundle.planArtifact, "tmp/vallum/live-proof-plan.json");
    assert.equal(bundle.liveStatusArtifact, "tmp/vallum/live-proof-status.json");
    ...
    await assertMode(join(cwd, "tmp/vallum/identity-proof-bundle.json"), 0o600);
    await assertMode(join(cwd, "tmp/vallum/live-proof-plan.json"), 0o600);
    await assertMode(join(cwd, "tmp/vallum/live-proof-status.json"), 0o600);
    await assertMode(join(cwd, "tmp/vallum/iota-names-live-report-template.json"), 0o600);
    await assertMode(join(cwd, "tmp/vallum/iota-identity-live-report-template.json"), 0o600);
    await assertMode(join(cwd, "tmp/vallum/vc-validation-live-report-template.json"), 0o600);
...
test("identity proof bundle is ready when names, identity, and VC gates are ready", async () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(liveStatusRaw).kind, "vallum.live-proof-status-report");
