...
export interface CustodyReadinessArtifact {
  ...
  readonly kind: "vallum.custody-readiness-report";
...
Reports current Vallum custody readiness without contacting custody systems or live signer infrastructure.
...
export function formatCustodyReadinessReport(report: CustodyReadinessReport): string {
  const lines = [
    `Vallum custody readiness ${report.productionReady ? "ready-for-approval" : "blocked"}`,
...
export function buildCustodyReadinessArtifact(
  ...
  return {
    schemaVersion: 1,
    kind: "vallum.custody-readiness-report",
...
async function checkLocalCustodyProof(
  ...
  if (!build.includes("npm run build -w @vallum/accounts")) missing.push("build @vallum/accounts");
...
function validateStructuredReport(
  ...
  if (report.kind !== "vallum.custody-production-proof") {
    return invalidReport("CUSTODY_PRODUCTION_REPORT_KIND_MISMATCH", "Production custody proof report has the wrong kind.", "configured-report-kind-mismatch", "Provide an vallum.custody-production-proof structured report.");
  }
