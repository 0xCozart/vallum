...
test("marketplace production proof plan reports current blockers without marketplace secrets", async () => {
  ...
  assert.ok(plan.requiredStructuredReportFields.includes("providerReview"));
  assert.ok(plan.requiredStructuredReportFields.includes("operationsReview"));
  assert.ok(plan.requiredStructuredReportCheckIds.includes("provider-verification-review"));
  assert.ok(plan.requiredStructuredReportCheckIds.includes("provider-capability-review"));
