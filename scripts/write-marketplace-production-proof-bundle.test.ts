...
test("marketplace production proof bundle writes template, plan, and blocked summary without marketplace secrets", async () => {
  ...
  try {
    ...
    const bundleRaw = await readFile(join(cwd, "tmp/gaskit/marketplace-production-proof-bundle.json"), "utf8");
    const planRaw = await readFile(join(cwd, "tmp/gaskit/marketplace-production-proof-plan.json"), "utf8");
    const readinessRaw = await readFile(join(cwd, "tmp/gaskit/marketplace-readiness.json"), "utf8");
    const templateRaw = await readFile(join(cwd, "tmp/gaskit/marketplace-production-report-template.json"), "utf8");

    assert.equal(bundle.kind, "agentic-gaskit.marketplace-production-proof-bundle");
    ...
    assert.deepEqual(bundle.templateArtifacts, [
      {
        ...
        path: "tmp/gaskit/marketplace-production-report-template.json",
        acceptedReportEnv: "MARKETPLACE_PRODUCTION_REPORT",
      },
    ]);
    assert.equal(bundle.planArtifact, "tmp/gaskit/marketplace-production-proof-plan.json");
    assert.equal(bundle.readinessArtifact, "tmp/gaskit/marketplace-readiness.json");
    ...
    await assertMode(join(cwd, "tmp/gaskit/marketplace-production-proof-bundle.json"), 0o600);
    await assertMode(join(cwd, "tmp/gaskit/marketplace-production-proof-plan.json"), 0o600);
    await assertMode(join(cwd, "tmp/gaskit/marketplace-readiness.json"), 0o600);
    await assertMode(join(cwd, "tmp/gaskit/marketplace-production-report-template.json"), 0o600);
...
async function writeMarketplaceEvidence(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), "agentic-gaskit-marketplace-proof-bundle-"));
  for (const path of [
