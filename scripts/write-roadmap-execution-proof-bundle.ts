...
export interface RoadmapExecutionProofBundle {
  ...
  readonly kind: "vallum.roadmap-execution-proof-bundle";
...
Writes one non-networked ignored local bundle for the remaining Vallum roadmap/operator proof gates.
...
export async function writeRoadmapExecutionProofBundle(
  ...
  const bundle: RoadmapExecutionProofBundle = {
    schemaVersion: 1,
    kind: "vallum.roadmap-execution-proof-bundle",
...
function readyCodesForStatusArtifact(
  ...
  if (artifact.kind === "vallum.product-status-report") return artifact.readyLiveCheckIds;
  if (artifact.kind === "vallum.operator-live-gate-report") return artifact.readyApprovalGateIds;
  if (artifact.kind === "vallum.launch-readiness-report") return artifact.provenLocalAreaIds;
  return [];
}
