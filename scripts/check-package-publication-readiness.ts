...
export interface PackagePublicationReadinessArtifact {
  ...
  readonly kind: "vallum.package-publication-readiness-report";
...
const PACKAGE_PUBLICATION_TEMPLATE_COMMAND = "npm run operator:write-report-template -- --kind package-publication --out tmp/vallum/package-publication-report-template.json";
...
Reports current Vallum package publication readiness without contacting npm or publishing packages.
...
export function formatPackagePublicationReadinessReport(report: PackagePublicationReadinessReport): string {
  const lines = [
    `Vallum package publication readiness ${report.liveReady ? "ready-for-approval" : "blocked"}`,
...
export function buildPackagePublicationReadinessArtifact(
  ...
  return {
    schemaVersion: 1,
    kind: "vallum.package-publication-readiness-report",
...
function validateStructuredReport(
  ...
  if (report.kind !== "vallum.package-publication-proof") {
    return invalidReport("PACKAGE_PUBLICATION_REPORT_KIND_MISMATCH", "Package publication proof report has the wrong kind.", "configured-report-kind-mismatch", "Provide an vallum.package-publication-proof structured report.");
  }
