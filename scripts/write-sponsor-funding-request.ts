...
export interface SponsorFundingRequest {
  ...
  readonly kind: "vallum.sponsor-funding-request";
...
const DEFAULT_OUT_FILE = "tmp/vallum/sponsor-funding-request.json";

const FAUCET_REPORT_ENV = "VALLUM_SPONSOR_FAUCET_REPORT";
...
If --faucet-report or VALLUM_SPONSOR_FAUCET_REPORT is set, only bounded sanitized faucet attempt metadata is copied into the ignored artifact.`;
...
export async function buildSponsorFundingRequest(
  ...
  return {
    schemaVersion: 1,
    kind: "vallum.sponsor-funding-request",
...
export function formatSponsorFundingRequestSummary(request: SponsorFundingRequest): string {
  return [
    "Vallum sponsor funding request",
    `written=true`,
    `kind=${request.kind}`,
    ...
    "next=Fund the public sponsor address from the ignored artifact, then run npm run sponsor:check-funding -- --report tmp/vallum/sponsor-funding-report.json.",
  ].join("\n");
}
