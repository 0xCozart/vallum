...
test("live proof status reports exact blockers without secret values", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "vallum-live-proof-"));
  try {
    ...
    assert.match(formatted, /VALLUM_SPONSOR_FUNDING_REPORT/);
    assert.match(formatted, /VALLUM_TESTNET_UPSTREAM_REPORT/);
    assert.match(formatted, /VALLUM_TESTNET_DIGEST_REPORT/);
    ...
    assert.match(formatted, /live:write-identity-proof-bundle -- --out tmp\/vallum\/identity-proof-bundle\.json/);
...
test("live proof status includes sanitized sponsor faucet failure context", async () => {
  ...
  try {
    ...
    const report = await checkLiveProofStatus({
      ...
      env: {
        VALLUM_SPONSOR_FUNDING_REPORT: "sponsor-funding-report.json",
        VALLUM_SPONSOR_FAUCET_REPORT: "sponsor-faucet-report.json",
      },
    });
    const formatted = formatLiveProofStatusReport(report);
    const funding = report.checks.find((check) => check.id === "sponsor-funding");
...
test("live proof status uses the default ignored sponsor faucet report when env is unset", async () => {
  ...
  try {
    ...
    await writeFile(join(cwd, "tmp/vallum/sponsor-faucet-request.json"), JSON.stringify({
      schemaVersion: 1,
      kind: "vallum.sponsor-faucet-request",
      result: "failed",
      code: "SPONSOR_FAUCET_FAILED",
    ...
    assert.doesNotMatch(formatted, /tmp\/vallum\/sponsor-faucet-request\.json|0x1234567890abcdef|faucet\.testnet\.example|task-/);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});
...
test("live proof status marks sponsored execute digest ready with a passing digest report", async () => {
  ...
  try {
    ...
    const report = await checkLiveProofStatus({
      ...
      gasStationRuntimeReport: readyGasStationRuntime(),
      env: {
        VALLUM_TESTNET_DIGEST_REPORT: "digest-report.json",
      },
    });
    const formatted = formatLiveProofStatusReport(report);
...
test("live proof status blocks configured identity proof endpoint without a smoke report", async () => {
  ...
  try {
    ...
    assert.match(identity?.next ?? "", /live:write-identity-proof-bundle/);
...
test("live proof status blocks VC trust policy config without an identity smoke report", async () => {
  ...
  try {
    ...
    assert.match(vc?.next ?? "", /live:write-identity-proof-bundle/);
...
test("live proof status blocks configured names without a smoke report", async () => {
  ...
  try {
    ...
    assert.match(names?.next ?? "", /live:write-identity-proof-bundle/);
...
test("live proof status artifact summarizes blockers without secret values", async () => {
  ...
  try {
    ...
    assert.equal(artifact.kind, "vallum.live-proof-status-report");
