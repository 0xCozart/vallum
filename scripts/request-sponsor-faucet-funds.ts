...
export interface SponsorFaucetRequestReport {
  ...
  readonly kind: "vallum.sponsor-faucet-request";
...
const DEFAULT_OUT_FILE = "tmp/vallum/sponsor-faucet-request.json";
const CHECK_SPONSOR_FUNDING_COMMAND = "npm run sponsor:check-funding -- --report tmp/vallum/sponsor-funding-report.json";
const DIAGNOSE_TESTNET_UPSTREAM_COMMAND = "npm run diagnose:gas-station -- --report tmp/vallum/testnet-upstream-diagnostic.json";
...
export async function requestSponsorFaucetFunds(
  ...
  const base = {
    ...
    kind: "vallum.sponsor-faucet-request" as const,
    observedAt,
...
export function validateSponsorFaucetRequestReport(
  ...
  if (report.schemaVersion !== 1 || report.kind !== "vallum.sponsor-faucet-request") {
...
function failedFaucetNextCommands(faucetReportPath: string): readonly string[] {
  return [
    `npm run sponsor:write-funding-request -- --faucet-report ${faucetReportPath} --out tmp/vallum/sponsor-funding-request.json`,
    CHECK_SPONSOR_FUNDING_COMMAND,
    `VALLUM_SPONSOR_FAUCET_REPORT=${faucetReportPath} npm run proof:live-status`,
  ];
}
