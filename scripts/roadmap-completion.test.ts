...
test("roadmap completion artifact summarizes blockers without configured values", () => {
  ...
  assert.equal(parsed.kind, "agentic-gaskit.roadmap-completion-audit");
...
test("roadmap completion artifact writer uses restrictive local file permissions", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "agentic-gaskit-roadmap-completion-"));
  try {
    const artifact = await writeRoadmapCompletionArtifact({
      cwd,
      now: new Date("2026-06-14T12:00:00.000Z"),
      outFile: "tmp/gaskit/roadmap-completion-audit.json",
      productStatus: productStatusFixture(),
      launchReadiness: launchReadinessFixture(),
      operatorLiveGates: operatorGatesFixture(),
    });
    const outFile = join(cwd, "tmp/gaskit/roadmap-completion-audit.json");
    const raw = await readFile(outFile, "utf8");
    const written = JSON.parse(raw) as typeof artifact;
    const mode = (await stat(outFile)).mode & 0o777;

    assert.equal(mode, 0o600);
    assert.equal(written.kind, "agentic-gaskit.roadmap-completion-audit");
    assert.equal(written.roadmapComplete, false);
    assert.deepEqual(written.blockerCodes, artifact.blockerCodes);
    assert.doesNotMatch(raw, /tmp\/gaskit\/roadmap-completion-audit\.json/);
  } finally {
  ...
  return {
    ...
    checks: [
      ...
      },
      {
        ...
        next: "npm run operator:write-report-template -- --kind iota-names-live --out tmp/gaskit/iota-names-live-report-template.json.",
      },
      {
  ...
  return {
    ...
    gates: [
      ...
      },
      {
        ...
        code: "IOTA_NAMES_LIVE_CONFIG_MISSING",
        command: "npm run operator:write-report-template -- --kind iota-names-live --out tmp/gaskit/iota-names-live-report-template.json && npm run smoke:iota-names-live -- --report <ignored-json-path>",
        approvalRequired: true,
        contactsLiveService: true,
        message: "IOTA Names live proof requires operator config.",
