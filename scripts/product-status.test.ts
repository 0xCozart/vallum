...
test("product status reports local proof gates and explicit live blockers without secrets", async () => {
  ...
  try {
    ...
    assert.match(formatted, /redacted payment-provider proof plan, readiness artifact, and report template/);
    ...
    assert.match(formatted, /npm run custody:write-production-proof-bundle -- --out <ignored-json-path>/);
    assert.match(formatted, /redacted custody production proof plan, readiness artifact, and report template/);
