...
function productionBlockers(): readonly ProductEvidenceCheck[] {
  return [
    ...
    },
    {
      ...
      message: "A2A discovery, task routes, authenticated extended Agent Card access, SSE streaming, push notification configuration, injected push delivery, opt-in push HTTP transport, callback URL admission hardening, callback host allowlisting, local retry/attempt observability, local durable attempt evidence, and local delivery queueing are proven locally and over loopback/local handler or mocked paths only; public readiness is reported locally and an opt-in public discovery/JWKS smoke can produce structured discovery evidence, but no public hosting run, production key distribution acceptance, valid operator-supplied structured public discovery report, valid structured public push delivery report, or valid structured external conformance proof is complete.",
