...
export interface ProductStatusArtifact {
  ...
  readonly kind: "vallum.product-status-report";
...
Reports current Vallum product status without contacting live proof services.
...
export function formatProductStatusReport(report: ProductStatusReport): string {
  const lines = [
    `Vallum product status ${report.complete ? "complete" : "not-complete"}`,
...
export function buildProductStatusArtifact(
  ...
  return {
    schemaVersion: 1,
    kind: "vallum.product-status-report",
