...
export function formatSponsorFundingReport(report: SponsorFundingReport): string {
  ...
  lines.push(report.ready
    ...
    : "next=Fund or consolidate the configured sponsor wallet on testnet, then rerun npm run sponsor:check-funding -- --report tmp/vallum/sponsor-funding-report.json.");
  return lines.join("\n");
}
